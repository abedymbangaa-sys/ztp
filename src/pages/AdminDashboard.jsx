...
import LocalPulseManager from "../components/admin/LocalPulseManager";
import { isCoreVerified } from "../lib/verificationStandard";
import { extractLatLng } from "../lib/mapUtils";
import { CheckCircle2, AlertTriangle } from "lucide-react";
import AdminStoryPinForm from "../components/AdminStoryPinForm";
import AdminHostForm from "../components/AdminHostForm";
import StarRating from "../components/StarRating";
import AdminPhotoSpotForm from "../components/AdminPhotoSpotForm";
import { AREAS } from "../data/areas";

// Small live-feedback line shown under the Google Maps Link field so
// whoever is entering a listing knows RIGHT AWAY whether the link they
// pasted will produce a pin on the map/Near Me, instead of finding out
// days later when a listing is silently missing from both. A
// maps.app.goo.gl short link or a plain text-search link (no
// coordinates in the URL) can't be read this way - the message tells
// them what to paste instead.
function MapsLinkHint({ value }) {
  ...
  if (coords) {
    return (
      <p className="mt-1.5 text-xs text-emerald-700 flex items-center gap-1">
        ...
        Coordinates detected - this will show a pin on the map and appear in "Near Me".
      </p>
    );
  }
  return (
    <p className="mt-1.5 text-xs text-amber-700 flex items-start gap-1">
      ...
      No coordinates found in this link - it won't get a map pin or show in "Near Me". Open the
      place in Google Maps, tap Share, then paste the full address-bar link (it should contain
      "@-6...,39..." or "query=-6...,39...") - not a maps.app.goo.gl short link.
    </p>
  );
}
...
const TABS = [
  ...
  { key: "site_reviews", label: "Site Reviews" },
  { key: "stories", label: "Traveler Stories" },
  { key: "categories", label: "Categories" },
  { key: "blog", label: "Blog" },
  { key: "itinerary", label: "Itinerary" },
  { key: "pulse", label: "Local Pulse" },
  { key: "story_map", label: "Story Map" },
  { key: "local_host", label: "Local Host" },
...
export default function AdminDashboard() {
  ...
  const [expandedReportsFor, setExpandedReportsFor] = useState(null);
  ...
  const [heroImagesMessage, setHeroImagesMessage] = useState("");
  const [heroImageSaving, setHeroImageSaving] = useState(false);
  const [heroImageMessage, setHeroImageMessage] = useState("");
  const [blogPosts, setBlogPosts] = useState([]);
  const [newPost, setNewPost] = useState({ title: "", excerpt: "", content: "", cover_image: "", language: "en", post_type: "blog", related_area: "" });
  ...
  async function loadAll() {
    ...
    const { data: siteReviewData } = await supabase
      .from("site_reviews")
      .select("*")
      .order("created_at", { ascending: false });
    setSiteReviews(siteReviewData || []);
    ...
    const { data: heroImagesData } = await supabase.from("settings").select("*").eq("key", "hero_images").single();
    try {
      setHeroImagesInput(heroImagesData?.value ? JSON.parse(heroImagesData.value) : []);
    } catch {
      setHeroImagesInput([]);
    }
  ...
  async function saveHeroImages() {
    ...
    setHeroImagesMessage("");
    const { error } = await supabase
      .from("settings")
      .upsert({ key: "hero_images", value: JSON.stringify(heroImagesInput) });
    setHeroImagesSaving(false);
    if (error) {
      setHeroImagesMessage("Error: " + error.message);
      return;
    }
    setHeroImagesMessage("Picha zimesaviwa! Homepage itazibadilisha kila baada ya sekunde chache.");
  }
  ...
  // Generic version of the toggle above, for the two suitability fields
  // (good_for / not_ideal_for) on the new-listing form.
  function toggleNewListingArrayField(field, key) {
    setNewListing((f) => ({
      ...f,
  ...
  // Same, for the edit-listing form.
  function toggleEditArrayField(field, key) {
    setEditForm((f) => ({
      ...f,
      [field]: (f[field] || []).includes(key) ? f[field].filter((k) => k !== key) : [...(f[field] || []), key],
    }));
  ...
  // Same idea as updateReviewStatus above, but for the site-wide "What
  // Visitors Say About Us" reviews - a separate table (site_reviews) that
  // wasn't getting a moderation UI before, so submissions sat stuck as
  // "pending" with no way to publish them.
  async function updateSiteReviewStatus(id, status) {
    await supabase.from("site_reviews").update({ status }).eq("id", id);
    loadAll();
  }

  // "Hide" is different from "Reject": reject is for a review that never
  // should have gone public (used from the pending queue); hide is for
  // pulling an already-published review back down - e.g. after a report
  // turns out to be valid - and always records why, per the review
  // policy's "removal must have a reason and audit trail" rule.
  async function hideReview(id) {
    const reason = window.prompt(
      "Reason for hiding this review (shown only to admins, not the public):"
    );
    if (reason === null) return; // cancelled
    await supabase.from("reviews").update({ status: "hidden", moderation_reason: reason }).eq("id", id);
    loadAll();
  }
  ...
  async function addBlogPost(e) {
    ...
    const payload = {
      ...
      related_area: newPost.related_area || null,
      status: "draft",
    };
  ...
  const [postEditForm, setPostEditForm] = useState(null);
  ...
  async function saveEditPost(e) {
    e.preventDefault();
    setPostEditMessage("Saving...");
    const { error } = await supabase.from("blog_posts").update(postEditForm).eq("id", editingPost.id);
    if (error) {
      setPostEditMessage("Error: " + error.message);
      return;
    }
    setEditingPost(null);
    loadAll();
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      {tab === "homepage" && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-xl mt-6">
          <h2 className="font-bold text-lg mb-2 text-slate-900">Hero Rotation (Picha Zinazobadilika)</h2>
          <p className="text-sm text-slate-500 mb-4">
            Ongeza picha kadhaa hapa - homepage itazizungusha moja baada ya nyingine (kila sekunde 6) badala ya
            kuonyesha picha moja tu. Ukiacha eneo hili wazi, "Hero Image" hapo juu ndiyo itaonekana peke yake bila
            kuzunguka. Tumia picha zenye ukubwa unaofanana (mf. 1600x900 au zaidi, mlalo/landscape) ili zisije
            zikaonekana zimekatwa vibaya.
          </p>
          <MultiPhotoUploader label="Picha za Hero" value={heroImagesInput} onChange={setHeroImagesInput} />
          <button
            onClick={saveHeroImages}
            disabled={heroImagesSaving}
            className="mt-4 bg-teal-700 hover:bg-teal-800 transition text-white font-bold px-6 py-2.5 rounded-full disabled:opacity-50"
          >
            {heroImagesSaving ? "Inasave..." : "Save Hero Rotation"}
          </button>
          {heroImagesMessage && (
            <p
              className={"text-sm mt-3 " + (heroImagesMessage.startsWith("Error") ? "text-red-600" : "text-teal-700")}
            >
              {heroImagesMessage}
            </p>
          )}
        ...
        <div className="space-y-3">
          <div className="bg-teal-50 border border-teal-200 rounded-2xl p-6 mb-2">
            ...
            <form onSubmit={handleAddListing} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Good For (optional)</label>
                <div className="flex flex-wrap gap-2">
                  {SUITABILITY_OPTIONS.map((s) => (
                    <button
                      key={s.key}
                      type="button"
                      onClick={() => toggleNewListingArrayField("good_for", s.key)}
                      className={
                        "inline-flex items-center gap-1.5 text-sm font-medium px-3.5 py-1.5 rounded-full border transition " +
                        ((newListing.good_for || []).includes(s.key)
                          ? "bg-teal-700 border-teal-700 text-white"
                          : "bg-white border-slate-300 text-slate-600 hover:border-teal-600")
                      }
                    >
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Not Ideal For (optional)</label>
                <div className="flex flex-wrap gap-2">
                  {SUITABILITY_OPTIONS.map((s) => (
                    <button
                      key={s.key}
                      type="button"
                      onClick={() => toggleNewListingArrayField("not_ideal_for", s.key)}
                      className={
                        "inline-flex items-center gap-1.5 text-sm font-medium px-3.5 py-1.5 rounded-full border transition " +
                        ((newListing.not_ideal_for || []).includes(s.key)
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                  />
                  <MapsLinkHint value={newListing.maps_link} />
                </div>
              </div>
              <button
                type="submit"
            ...
            <div key={l.id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {l.status === "approved" && !l.whatsapp_number && (
                    <p className="text-xs text-red-600 font-semibold mt-0.5">
                      No WhatsApp number - can't receive direct leads
                    </p>
                  )}
                </div>
              </div>
            ...
            <form onSubmit={saveEditListing} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                />
                <MapsLinkHint value={editForm.maps_link} />
              </div>
              ...
              {/* Good for / Not ideal for - suitability guidance shown on
                  the card and detail page. All optional. */}
              <div className="border border-slate-200 rounded-lg p-4 space-y-3">
                <p className="text-sm font-semibold text-slate-700">Good For / Not Ideal For (optional)</p>
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Good for</label>
                  <div className="flex flex-wrap gap-2">
                    {SUITABILITY_OPTIONS.map((s) => (
                      <button
                        key={s.key}
                        type="button"
                        onClick={() => toggleEditArrayField("good_for", s.key)}
                        className={
                          "inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border transition " +
                          ((editForm.good_for || []).includes(s.key)
                            ? "bg-teal-700 border-teal-700 text-white"
                            : "bg-white border-slate-300 text-slate-600 hover:border-teal-600")
                        }
                      >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Not ideal for</label>
                  <div className="flex flex-wrap gap-2">
                    {SUITABILITY_OPTIONS.map((s) => (
                      <button
                        key={s.key}
                        type="button"
                        onClick={() => toggleEditArrayField("not_ideal_for", s.key)}
                        className={
                          "inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border transition " +
                          ((editForm.not_ideal_for || []).includes(s.key)
                            ? "bg-slate-700 border-slate-700 text-white"
                            : "bg-white border-slate-300 text-slate-600 hover:border-slate-500")
                        }
                      >
      ...
      {tab === "reviews" && (
        <div className="space-y-3">
          ...
          {reviews.map((r) => {
            const reportsForThis = reviewReports.filter((rep) => rep.review_id === r.id);
            return (
            <div key={r.id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {r.helpful_count > 0 && (
                    <span className="text-xs font-semibold text-slate-500">👍 {r.helpful_count}</span>
                  )}
                  {reportsForThis.length > 0 && (
                    <button
                      onClick={() => setExpandedReportsFor(expandedReportsFor === r.id ? null : r.id)}
                      className="text-xs font-semibold text-red-700 bg-red-50 px-2 py-0.5 rounded-full hover:bg-red-100"
                    >
                      🚩 {reportsForThis.length} report{reportsForThis.length === 1 ? "" : "s"}
                    </button>
                  )}
                </div>
                <p className="text-xs text-slate-500 mb-2">
                  Regarding: {r.listings?.title || "—"}
                  {r.trip_type ? ` · ${r.trip_type} trip` : ""}
                </p>
                ...
                {r.owner_response && (
                  <div className="mt-2 bg-slate-50 border border-slate-200 rounded-lg p-2">
                    <p className="text-xs font-semibold text-slate-600">Owner response:</p>
                    <p className="text-xs text-slate-600">{r.owner_response}</p>
                  </div>
                )}
                {r.status === "hidden" && r.moderation_reason && (
                  <p className="text-xs text-slate-400 mt-2 italic">Hidden reason: {r.moderation_reason}</p>
                )}
                {expandedReportsFor === r.id && (
                  <div className="mt-2 space-y-1.5">
                    {reportsForThis.map((rep) => (
                      <div key={rep.id} className="bg-red-50 border border-red-100 rounded-lg p-2">
                        <p className="text-xs font-semibold text-red-700">{rep.reason}</p>
                        {rep.note && <p className="text-xs text-red-600">{rep.note}</p>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="flex items-center gap-2 shrink-0 flex-wrap">
                ...
                {r.status === "approved" && (
                  <button
                    ...
                    className="text-xs font-semibold bg-white text-red-600 border border-red-300 px-3 py-1.5 rounded-full hover:bg-red-50"
                  >
                    Hide
                  </button>
                )}
                {r.status === "hidden" && (
                  <button
                    onClick={() => updateReviewStatus(r.id, "approved")}
                    className="text-xs font-semibold bg-white text-teal-700 border border-teal-300 px-3 py-1.5 rounded-full hover:bg-teal-50"
                  >
                    Restore
                  </button>
                )}
      ...
      {tab === "site_reviews" && (
        <div className="space-y-3">
          <p className="text-sm text-slate-500 mb-2">
            Reviews submitted about the website itself (from the "What Visitors Say About Us" section on the
            homepage). Approve to make them public.
          </p>
          {siteReviews.length === 0 && <p className="text-sm text-slate-400">No site reviews yet.</p>}
          {siteReviews.map((r) => (
            <div key={r.id} className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="flex gap-2 mt-3">
                {r.status !== "approved" && (
                  <button
                    onClick={() => updateSiteReviewStatus(r.id, "approved")}
                    className="text-xs font-semibold text-white bg-teal-700 hover:bg-teal-800 px-3 py-1.5 rounded-full"
                  >
                    Approve
                  </button>
                )}
                {r.status !== "rejected" && (
                  <button
                    onClick={() => updateSiteReviewStatus(r.id, "rejected")}
                    className="text-xs font-semibold text-red-700 border border-red-300 hover:bg-red-50 px-3 py-1.5 rounded-full"
                  >
                    Reject
                  </button>
                )}
      ...
      {tab === "story_map" && <AdminStoryPinForm />}

      {tab === "local_host" && <AdminHostForm />}

      {tab === "photo_spots" && <AdminPhotoSpotForm />}
            ...
            <form onSubmit={addBlogPost} className="space-y-3">
              ...
              <div>
                <label className="block text-sm text-slate-500 mb-1">
                  Related area <span className="text-slate-400">(optional - shows relevant listings at the end of the post)</span>
                </label>
                <select
                  value={newPost.related_area}
                  onChange={(e) => setNewPost({ ...newPost, related_area: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                >
              ...
              <SinglePhotoUploader
                ...
                onChange={(url) => setNewPost({ ...newPost, cover_image: url })}
              />
              <textarea
      ...
      {editingPost && postEditForm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setEditingPost(null)}>
          <div
            className="bg-white rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setEditingPost(null)} className="text-slate-400 hover:text-slate-700 text-xl leading-none">
                &times;
              </button>
            </div>
            <form onSubmit={saveEditPost} className="space-y-3">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Title</label>
                <input
                  value={postEditForm.title}
                  onChange={(e) => setPostEditForm({ ...postEditForm, title: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                  required
                />
              </div>
              <div>
                ...
                <input
                  value={postEditForm.excerpt}
                  onChange={(e) => setPostEditForm({ ...postEditForm, excerpt: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                />
              </div>
              <SinglePhotoUploader
                label="Cover Photo"
                value={postEditForm.cover_image}
                onChange={(url) => setPostEditForm({ ...postEditForm, cover_image: url })}
              />
              <div>
                <label className="block text-sm text-slate-500 mb-1">
                  Related area <span className="text-slate-400">(optional - shows relevant listings at the end of the post)</span>
                ...
                <select
                  value={postEditForm.related_area}
                  onChange={(e) => setPostEditForm({ ...postEditForm, related_area: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                >
                  <option value="">— None —</option>
                  {AREAS.map((a) => (
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setPostEditForm({ ...postEditForm, content: e.target.value })}
                  className="w-full border border-slate-300 rounded-lg px-4 py-2.5"
                  required
                />
              </div>
              {postEditMessage && <p className="text-sm text-slate-600">{postEditMessage}</p>}
              <button
                type="submit"
                className="w-full bg-teal-700 hover:bg-teal-800 transition text-white font-bold px-6 py-2.5 rounded-full"
              >
                Save Changes
              </button>
            </form>
